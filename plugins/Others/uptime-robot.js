...
const handler = async (m, { conn, command, text }) => {
  const apiKey = process.env.UPTIMEROBOT_API_KEY; // Modified to use an environment variable
      ...
      if ("new" === action) {
        ...
        try {
          ...
          console.log(response),
            "ok" === response.stat && response.monitor?.id
              ? m.reply(
                  `✅ Monitor baru berhasil dibuat dengan ID: ${response.monitor.id}`,
                )
        ...
        try {
          ...
          console.log(response),
            "ok" === response.stat && response.monitor?.id
              ? m.reply(
                  `✅ Monitor dengan ID: ${response.monitor.id} berhasil dihapus.`,
                )
        ...
        m.reply(
          "❌ Aksi tidak valid! Gunakan `new|input`, `del|input`, atau `stats`.",
        );
    } else
      m.reply(
        "❌ Harap berikan perintah yang valid! Gunakan `new|input`, `del|input`, atau `stats`.",
      );
};
