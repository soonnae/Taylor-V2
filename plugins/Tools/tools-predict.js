...
const handler = async (m, { conn, args, text, usedPrefix, command }) => {
  ...
  /webp/g.test(mime)
    ...
    : /image/g.test(mime)
      ...
      : (/video/g.test(mime) || /gif/g.test(mime) || /viewOnce/g.test(mime)) &&
        (out = await uploadFile(img)),
    m.react(wait);
  try {
    const apiResponse = await analyzeImage(out),
      { items } = apiResponse.api4ai,
      nsfwScore =
        100 *
        (items.find((item) => "nsfw" === item.label) || {}).likelihood_score,
      sfwScore =
        100 *
        (items.find((item) => "sfw" === item.label) || {}).likelihood_score;
    m.reply(
