...
const weather = async (text) => {
  try {
    ...
    const geocodingResponse = await fetch(
      `${GEOCODING_URL}?${geocodingParams}`,
    );
    if (!geocodingResponse.ok)
      throw new Error("Gagal mendapatkan data geocoding");
    ...
    const sunriseTime = new Date(sys?.sunrise * 1e3).toLocaleTimeString(
      "id-ID",
    );
...
const handler = async (m, { conn, text }) => {
  ...
  try {
    ...
    m.react(wait);
    let { caption, iconUrl, tileUrl } = await weather(text);
