...
const getLocationData = async (query) => {
  try {
    const response = await fetch(
      `${LOCATION_URL}?q=${query}&apikey=${ACCUWEATHER_API_KEY}&language=id-id`,
    );
...
const getForecastData = async (locationKey) => {
  try {
    const response = await fetch(
      `${FORECAST_URL}/${locationKey}?apikey=${ACCUWEATHER_API_KEY}&details=true&language=id-id`,
    );
    if (!response.ok) throw new Error("Gagal mendapatkan data perkiraan cuaca");
    return await response.json();
  } catch (error) {
    throw new Error(
      `Error saat mengambil data perkiraan cuaca: ${error.message}`,
    );
  }
};
...
const weather = async (text) => {
  try {
    ...
    const { Key, LocalizedName, Country } = locationData;
    ...
    const forecastList =
      forecastData.DailyForecasts?.slice(0, 5)
        .map((day) => {
          const date = new Date(day.Date).toLocaleDateString("id-ID");
          const maxTempCelsius = fahrenheitToCelsius(
            day.Temperature?.Maximum?.Value ?? 0,
          );
          const minTempCelsius = fahrenheitToCelsius(
            day.Temperature?.Minimum?.Value ?? 0,
          );
          return `${date}: ${maxTempCelsius}°C, ${day.Day?.IconPhrase ?? "Tidak tersedia"}`;
        })
        .join("\n") ?? "Tidak tersedia";
    const airQuality = forecastData.DailyForecasts?.[0]?.AirAndPollen
      ? forecastData.DailyForecasts[0].AirAndPollen.map(
          (pollen) => `${pollen.Name}: ${pollen.Category}`,
        ).join(", ")
      : "Tidak tersedia";
    const realFeelMaxCelsius = fahrenheitToCelsius(
      today.RealFeelTemperature?.Maximum?.Value ?? 0,
    );
    const realFeelMinCelsius = fahrenheitToCelsius(
      today.RealFeelTemperature?.Minimum?.Value ?? 0,
    );
...
const handler = async (m, { conn, text }) => {
  ...
  try {
    ...
    m.react(wait);
    let { caption, iconBuffer } = await weather(text);
    await conn.sendFile(
      m.chat,
      iconBuffer,
      "weather-icon.png",
