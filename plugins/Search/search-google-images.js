...
const handler = async (m, { command, usedPrefix, conn, text, args }) => {
  ...
  if (command === "googleimg") {
    try {
      const param = {
        api_key: process.env.GOOGLE_IMAGES_API_KEY, // Load API key from environment variable
...
async function GoogleSearch(q, search_type) {
  try {
    const params = {
      api_key: process.env.VALUE_SERP_API_KEY, // Load API key from environment variable
      q: q,
