...
class FineShare {
  spinner;
  token = `Bearer ${process.env.FINESHARE_API_TOKEN}`;
  constructor() {
    this.spinner = ora({
      text: "FineShare",
      spinner: "moon",
  ...
  async voiceDetail(voicename) {
    try {
      ...
      (this.spinner.text =
        chalk.yellow.bold("Process:") +
        chalk.white(" Detail Voice request...")),
        this.spinner.render();
      const response = await fetch(url, options),
        data = await response.json();
      return (
        this.spinner.succeed(
          chalk.green.bold("Success:") + chalk.white(" Detail Voice request."),
        ),
        data || {}
      );
    } catch (error) {
  ...
  async myVoice(page) {
    try {
      ...
      (this.spinner.text =
        chalk.green.bold("Process:") +
        chalk.white(" List My Voice request...")),
        this.spinner.render();
      const response = await fetch(url, options),
        data = await response.json();
      return (
        this.spinner.succeed(
          chalk.green.bold("Success:") + chalk.white(" List My Voice request."),
        ),
        data || {}
      );
    } catch (error) {
  ...
  async pageVoice(category, gender, language, page, limit) {
    ...
    try {
      ...
      (this.spinner.text =
        chalk.green.bold("Process:") +
        chalk.white(" List Page Voice request...")),
        this.spinner.render();
      const response = await fetch(url, options),
        data = await response.json();
      return (
  ...
  async voiceRemove(uuid) {
    try {
      ...
      (this.spinner.text =
        chalk.blue.bold("Process:") + chalk.white(" Voice Remove request...")),
        this.spinner.render();
      const response = await fetch(url, options),
        data = await response.json();
      return (
        this.spinner.succeed(
          chalk.green.bold("Success:") + chalk.white(" Voice Remove request."),
        ),
        data
      );
    } catch (error) {
  ...
  async userUsage() {
    try {
      ...
      (this.spinner.text =
        chalk.blue.bold("Process:") + chalk.white(" User Usage request...")),
        this.spinner.render();
      const response = await fetch(url, options),
        data = await response.json();
      return (
        this.spinner.succeed(
          chalk.green.bold("Success:") + chalk.white(" User Usage request."),
        ),
        data
      );
    } catch (error) {
  ...
  async voiceChanger(voicename, audiofile) {
    try {
      ...
      (this.spinner.text =
        chalk.blue.bold("Process:") + chalk.white(" Voice Changer request...")),
        this.spinner.render();
      const response = await fetch(url, options),
        data = await response.json();
      return (
        this.spinner.succeed(
          chalk.green.bold("Success:") + chalk.white(" Voice Changer request."),
        ),
        await this.voiceUpload(
          data.uuid,
          data.endpoint,
          audiofile,
          voicename,
  ...
  async voiceUpload(uuid, host, audio, name, type) {
    try {
      ...
      (this.spinner.text =
        chalk.magenta.bold("Process:") +
        chalk.white(" Voice Upload request...")),
        this.spinner.render();
      const response = await fetch(url, options);
      await response.json();
      return (
        this.spinner.succeed(
          chalk.green.bold("Success:") + chalk.white(" Voice Upload request."),
        ),
  ...
  async voiceStart(uuid, voice, pitch) {
    try {
      ...
      (this.spinner.text =
        chalk.cyan.bold("Process:") + chalk.white(" Voice Start request...")),
        this.spinner.render();
      const response = await fetch(url, options);
      await response.json();
      return (
        this.spinner.succeed(
          chalk.green.bold("Success:") + chalk.white(" Voice Start request."),
        ),
  ...
  async getvoiceResult(uuid) {
    ...
    do {
      try {
        ...
        (this.spinner.text =
          chalk.red.bold("Process:") +
          chalk.white(" Get Voice Result request...")),
          this.spinner.render();
        const response = await fetch(url, options),
          data = await response.json();
        data.url
    ...
    return (
      dataUrl
        ...
        : this.spinner.fail(
            "Exceeded maximum attempts. Unable to retrieve URL.",
          ),
