...
class StableHorde {
  constructor({ apiKey = "" }) {
    (this.apiKey = apiKey || process.env.STABLE_HORDE_API_KEY),
  ...
  async generateText(dataInput) {
    const headers = {
      ...
      body = JSON.stringify({
        prompt:
          dataInput.prompt +
          (dataInput.negativePrompt ? ` ### ${dataInput.negativePrompt}` : ""),
        params: {
          ...dataInput,
        },
    ...
    if (401 === response.status)
      return console.log(chalk.yellow("Unauthorized access:"), response), null;
...
class AiHorde {
  constructor({ apiKey = "" }) {
    (this.apiKey = apiKey || process.env.AI_HORDE_API_KEY),
      (this.pendingTextGenerationIds = []),
      (this.pendingImageGenerationIds = []),
      (this.startTime = 0);
  }
