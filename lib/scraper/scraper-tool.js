...
async function sifat_usaha_bisnis(tgl, bln, thn) {
  return new Promise((resolve, reject) => {
    axios({
      ...
      let hasil,
        fetchText = cheerio.load(data)("#body").text();
      try {
        hasil = {
          status: !0,
          message: {
...
async function remini(url, apikey) {
  ...
  for (let i = 0; i < 50; i++) {
    ...
    if ("completed" === getTaskResponse.data.status)
      return getTaskResponse.data.result.output_url;
    if ("processing" !== getTaskResponse.data.status)
...
async function ttslide(text) {
  const html = (
      await axios.get(`https://dlpanda.com/id?url=${text}&token=G7eRpMaa`)
    ).data,
    $ = cheerio.load(html);
  let asd = [],
    imgSrc = [];
    ...
    axios
      ...
      .then((res) => {
        const $ = cheerio.load(res.data),
          ...
          token_name = $("#submit-form > div")
            .find("div:nth-child(1) > input[type=hidden]:nth-child(2)")
            .attr("name"),
          token_ = $("#submit-form > div")
            .find("div:nth-child(1) > input[type=hidden]:nth-child(2)")
            .attr("value"),
          verify = $("#submit-form > div")
