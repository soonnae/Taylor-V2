...
const trueCallerModels = [
  ...
  device =
    trueCallerModels[Math.floor(Math.random() * trueCallerModels.length)];
...
export async function TrueLogin(phoneNumber) {
  ...
  const options = {
    ...
    headers: {
      ...
      clientsecret: process.env.TRUECALLER_CLIENT_SECRET, // Use environment variable
    },
