...
const _accessToken = process.env.LUMA_ACCESS_TOKEN; // Use environment variable
const headers = {
  ...
  Svgai = async (prompt) => {
    try {
      const { data } = await axios.post(
          "https://api.svg.io/generate-image",
          {
            userPrompt: prompt,
          },
          {
  ...
  MyInstant = class {
      ...
      try {
        ...
        return $(".instant")
          .map((_, el) => ({
            ...
            soundLink: `https://www.myinstants.com${
              $(el)
    ...
    async category(cat) {
      ...
      return categories.includes(cat)
        ? await this.scrape(`https://www.myinstants.com/en/categories/${cat}`)
        : (console.error("Invalid category."), categories);
    }
    async new() {
  ...
  Zmoai = async (
    prompt,
    identify = process.env.ZMOAI_IDENTIFY, // Use environment variable
    ...
    try {
      ...
      const { batchTaskId } = await createResponse.json(),
        urlGet = `https://web-backend-prod.zmo.ai/api/v1.0/microTask/makeUp/get?batchTaskId=${batchTaskId}`;
      let taskDetails,
        attempts = 0;
  ...
  Arthub = async (
    ...
    const headers = {
        "Content-Type": "application/json",
        Authorization: `Bearer ${process.env.ARTHUB_TOKEN}`, // Use environment variable
      },
      data = {
    ...
    try {
      ...
      const { id: batchTaskId } = await createResponse.json(),
        urlGet = `https://arthub-gen-worker.repalash.workers.dev/api/v1/check/${batchTaskId}`;
      let taskDetails,
        attempts = 0;
      const maxAttempts = 20;
      do {
...
async function LumaAi(
  prompt = _prompt,
  imgFile = null,
  accessToken = _accessToken,
) {
  const url =
    "https://internal-api.virginia.labs.lumalabs.ai/api/photon/v1/generations/";
  const payload = {
    user_prompt: prompt,
    aspect_ratio: "16:9",
    expand_prompt: true,
  ...
  do {
    ...
    const getResponse = await fetch(
      `https://internal-api.virginia.labs.lumalabs.ai/api/photon/v1/generations/${result[0]?.id}`,
      {
        method: "GET",
        headers: headers,
      },
    ...
    if (!getResponse.ok)
      throw new Error(`Error getting task details: ${getResponse.state}`);
    (taskDetails = await getResponse.json()), (attempts += 1);
...
async function LumaAiRefresh(accessToken = _accessToken) {
  const url =
    "https://internal-api.virginia.labs.lumalabs.ai/api/photon/v1/user/generations/";
...
async function LumaAiSign(accessToken = _accessToken) {
  const url =
    "https://internal-api.virginia.labs.lumalabs.ai/api/photon/v1/generations/file_upload";
  const params = new URLSearchParams({
    file_type: "image",
    filename: "file.jpg",
  });
...
async function LumaAiFile(filePath, accessToken = _accessToken) {
  try {
    const { presigned_url, public_url } = await LumaAiSign(accessToken);
