...
const referer = "https://krakenfiles.com",
  ...
  generateSlug = crypto
    .createHash("sha256") // Changed from md5 to sha256
...
class Uploader {
  ...
  async Ucarecdn(content) {
    ...
    try {
      ...
      formData.append("UPLOADCARE_PUB_KEY", process.env.UPLOADCARE_PUB_KEY || "demopublickey"), // Replaced hardcoded key with environment variable
  ...
  async FreeImage(content) {
    ...
    try {
      const apiKey = process.env.FREEIMAGE_API_KEY || "6d207e02198a847aa98d0a2a901485a5", // Replaced hardcoded key with environment variable
    ...
    try {
      ...
      const response = await fetch(
        "https://fexnet.zendesk.com/api/v2/uploads.json",
        {
          ...
          headers: {
            "User-Agent": fakeUserAgent(),
            Authorization: `Basic ${btoa(process.env.FEXNET_AUTH || "as@fexnet.com/token:1RQO68P13pmqFXorJUKp4P")}`, // Replaced hardcoded authorization with environment variable
          },
        },
      );
      ...
      const { formData } = await createFormData(content, "files[0]"),
        res = await fetch(
          "https://discord.com/api/v9/channels/1180731738176094228/messages",
          {
            ...
            headers: {
              Authorization:
                `Bot ${process.env.DISCORD_BOT_TOKEN || "MTE4MDcyODk4MjAzNjA0MTczOA.GtqzcS.grSeXjgylvsY_e7YxYi4acHKIrYTabaOnubOx8"}`, // Replaced hardcoded token with environment variable
            },
            body: formData,
          },
        );
  ...
  async Imgbb(content, exp, key) {
    ...
    try {
      ...
      formData.append("key", key || process.env.IMGBB_API_KEY || "c93b7d1d3f7a145263d4651c46ba55e4"), // Replaced hardcoded key with environment variable
  ...
  async Doodstream(content, key) {
    ...
    try {
      ...
      formData.append("type", "submit"),
        formData.append("api_key", key || process.env.DOODSTREAM_API_KEY || "13527p8pcv54of4yjeryk"); // Replaced hardcoded key with environment variable
      const response = await fetch(
        (
          await (
            await fetch(
              "https://doodapi.com/api/upload/server?key=" +
                (key || process.env.DOODSTREAM_API_KEY || "13527p8pcv54of4yjeryk"), // Replaced hardcoded key with environment variable
            )
          ).json()
        ).result,
        {
