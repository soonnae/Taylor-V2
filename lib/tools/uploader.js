...
const referer = "https://krakenfiles.com",
  ...
  generateSlug = crypto
    .createHash("sha256")  // Changed from md5 to sha256 for secure hashing
      ...
      const { formData } = await createFormData(content, "files[0]"),
        res = await fetch(
          "https://discord.com/api/v9/channels/1180731738176094228/messages",
          {
            ...
            headers: {
              Authorization:
                `Bot ${process.env.DISCORD_BOT_TOKEN}`,  // Replaced hardcoded token with environment variable
            },
            body: formData,
          },
        );
          ...
          await (
            await fetch(
              "https://doodapi.com/api/upload/server?key=" +
                (key || process.env.DOODSTREAM_API_KEY || "13527p8pcv54of4yjeryk"),
            )
          ).json()
        ).result,
        {
