...
class ShortLink {
  ...
  async tnyim(url) {
    try {
      const response = await fetch(
        `https://tny.im/yourls-api.php?format=json&action=shorturl&url=${url}`,
      );
      return (await response.json()).shorturl;
    } catch (error) {
      return console.error(error), null;
  ...
  async shrtco(url) {
    try {
      const response = await fetch(
        ...
        result = await response.json(),
        linkArray = Object.values(result.result).filter(
  ...
  async adfoc(url) {
    try {
      const response = await fetch(
        `https://adfoc.us/api/?key=3dda30eb032141a2b2b3100a3dbfb3c9&url=${url}`,
      );
      return await response.text();
    } catch (error) {
      return console.error(error), null;
  ...
  async dxyz(url) {
    try {
      return await (
        await fetch("https://d.131213.xyz/create", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
  ...
  async shorturl(url) {
    try {
      ...
      return (await response.text())
        .split('id="shortenurl" type="text" value="')[1]
        .split('"')[0];
    } catch (error) {
      return console.error(error), null;
    }
  ...
  async adfly(url) {
    try {
      const keys = [
          "9a283dafe534bddccc556153a37ed678",
