...
export default class Spotify {
  ...
  async getTask(str) {
    try {
      const parse = await axios.get(
          "https://ytmp3api.net/iframe/?color=green&vid=48MUdbh6B0w",
        ),
        p = {
      ...
      const html = await axios.post(
          "https://ytmp3api.net/iframe/convert.php",
          form,
          {
            headers: {
              Accept: "*/*",
  ...
  async fetch(url) {
    try {
      var id = (
        /track/.test(url) ? url.split("track/")[1] : url.split("playlist/")[1]
      ).trim();
      if (/playlist/.test(url)) {
        ...
        if (!json.data.success || json.data.trackList.length < 1)
          return {
            creator: "Wudysoft",
            status: !1,
